...
- **Fixed Holdings Component Authentication and Display Issues**: Resolved critical authentication rate-limiting issue where Holdings component was making redundant `/api/auth/user` calls causing 429 errors and "Failed to load holdings" after initial display. Removed extra authentication calls and simplified to use session credentials directly. Fixed account name mapping across all components (Connected Accounts, Unified Dashboard, Holdings) to properly display "Coinbase" instead of "Default" for Coinbase accounts. Updated both frontend display logic and backend brokerage name mapping for consistent account identification. Holdings now load reliably without authentication errors and display correct account names throughout the application.
