...
export const rateLimits = {
  ...
  auth: createRateLimit({
    ...
    maxRequests: 5,
    keyGenerator: (req) => `auth:${req.ip}:${req.user?.claims?.sub || 'anonymous'}`
  }),
  ...
  trading: createRateLimit({
    ...
    maxRequests: 30,
    keyGenerator: (req) => `trading:${req.user?.claims?.sub || req.ip}`
  }),
  ...
  data: createRateLimit({
    windowMs: 60 * 1000, // 1 minute
    maxRequests: 100,
    keyGenerator: (req) => `data:${req.user?.claims?.sub || req.ip}`
  }),
  ...
  external: createRateLimit({
    windowMs: 60 * 1000, // 1 minute
    maxRequests: 10,
    keyGenerator: (req) => `external:${req.user?.claims?.sub || req.ip}`
  })
};