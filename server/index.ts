...
process.env.SNAPTRADE_CLIENT_ID = clean(process.env.SNAPTRADE_CLIENT_ID);
process.env.SNAPTRADE_CONSUMER_KEY = clean(process.env.SNAPTRADE_CONSUMER_KEY);
process.env.SNAPTRADE_ENV = clean(process.env.SNAPTRADE_ENV);
process.env.SNAPTRADE_REDIRECT_URI = clean(process.env.SNAPTRADE_REDIRECT_URI);
...
console.log('[ENV CHECK]', bootEnv);

// HARD GUARDS to prevent 1076-causing misconfig
if (!bootEnv.CLIENT_ID || !bootEnv.CONSUMER_KEY_LEN) {
  throw new Error('SnapTrade config missing: CLIENT_ID or CONSUMER_KEY');
}
if (bootEnv.ENV !== 'sandbox') {
  throw new Error(`SnapTrade ENV must be 'sandbox' right now; got '${bootEnv.ENV}'`);
}
if (!/^FLINT-TEST-/.test(bootEnv.CLIENT_ID)) {
  throw new Error(`CLIENT_ID '${bootEnv.CLIENT_ID}' does not look like your sandbox app (expected prefix FLINT-TEST-)`);
}
if (!/^https:\/\/.+\.kirk\.replit\.dev\/snaptrade\/callback$/.test(bootEnv.REDIRECT || '')) {
  throw new Error(`SNAPTRADE_REDIRECT_URI must be your public Replit URL (no :5000) and include /snaptrade/callback`);
}
